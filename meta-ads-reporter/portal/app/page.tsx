...
async function getReports(): Promise<ReportEntry[]> {
  try {
    const { blobs } = await list({ prefix: "reports/index.json" });
  ...
  return (
    <main style={{ maxWidth: 780, margin: "0 auto", padding: "32px 16px" }}>
      ...
      <div style={{
        ...
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>📊 Relatórios Meta Ads</h1>
        <p style={{ margin: "6px 0 0", opacity: 0.85, fontSize: 14 }}>
          Acompanhe o desempenho das suas campanhas semanalmente
        </p>
      </div>
          ...
          {reports.map((r) => (
            <div key={r.id} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: "#1C1E21" }}>
                    {r.label}
                  </h2>
